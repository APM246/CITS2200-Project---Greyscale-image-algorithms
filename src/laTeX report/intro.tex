	...
	\begin{equation}
     ...
     \end{tabular} \newline \newline
      It then finds the sum of every possible subcolumn of length k from each column in \textit{row\_sums}. The maximum of these sums is the brightness of the brightest square and is the returned answer. \newline
       ...
       search is based on one property. The brightness of a path is defined by the brightest pixel it contains. Therefore, when examining a new pixel to add to the path, there are two possible outcomes. Either the pixel redefines the path's brightest pixel by being brighter than the brightest pixel encountered thus far or the pixel joins the path without affecting this property. This is seen with the conditional statement:\newline
       \begin{lstlisting}
       ...
       \end{lstlisting}
        Hence the key array in the algorithm always contains an upper bound on the lowest brightness a pixel has encountered. Due to the nature of a priority first search algorithm whereby higher priority elements (darker pixels) are dequeued first from the priority queue, the algorithm is guaranteed to construct a tree which emphasises on darkest paths. Once (vr, vc) is dequeued from the heap, 
        its upper bound will have converged to the lowest possible value and is therefore returned. 
