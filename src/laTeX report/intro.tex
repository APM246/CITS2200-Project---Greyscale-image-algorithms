     ...
     \end{tabular} \newline \newline
      It then finds the sum of every possible subcolumn of length k from each column in \textit{row\_sums}. The maximum of these sums is the brightness of the brightest square and is the returned answer. \newline
       ...
       \end{lstlisting}
		...
		Once a binary tree or segment tree is constructed from a row, queries can be solved using a recursive divide and conquer algorithm. With each call of the function, one of 5 outcomes (4 base case + 1 recursive call) occurs:\newline
		...
		4) The range of the node covers 1 index, thus return the node's value\\
